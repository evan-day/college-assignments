//Process Four - Behaves as Incoming Bay Three. Using actuators, it checks what the level of the truck is. If empty, it states it's empty.
...
void myHandler(int sig, siginfo_t *info, void *other)
   ...
   switch (info->si_value.sival_int) {
      default:
    	  	  	ActuatorSetValue(3, y);
    	  	  	if (y == 0) {
    	  	  		printf("Incoming Bay Three: Truck is empty and ready to depart\n");
    	  	  	} else {
    	  	  		printf("Incoming Bay Three: Unloading from truck\n");
    	  	  		y = y - 25;
    	  	  		z = z + 25;
...
ActuatorSetName(3, "Truck Three Status");
