//Process Two - Behaves as Incoming Bay One. Using actuators, it checks what the level of the truck is. If empty, it states it's empty.
...
void myHandler(int sig, siginfo_t *info, void *other)
   ...
   switch (info->si_value.sival_int) {
      default:
    	  	  	ActuatorSetValue(1, y);
    	  	  	if (y == 0) {
    	  	  		printf("Incoming Bay One: Truck is empty and ready to depart\n");
    	  	  	} else {
    	  	  		printf("Incoming Bay One: Unloading from truck\n");
    	  	  		y = y - 25;
    	  	  		z = z + 25;
...
ActuatorSetName(1, "Truck One Status");
