...
public class MyClient extends Application implements ClientInterface {
	private DataInputStream controlTrafficFromServer;
	private DataOutputStream controlTrafficToServer;

	private ObjectInputStream physicalTrafficFromServer;
	private ObjectOutputStream physicalTrafficToServer;
	...
	private static File target = new File("H:\\EmbeddedButDistributed\\EmbeddedButDistributed\\src\\local-server\\");
	...
	public void start(Stage primaryStage) {
		...
		try {
			...
			//observerClientSocket = new Socket(serverHost, observerServerPort);
			
			controlTrafficFromServer = new DataInputStream(server.getInputStream());
			//observerControlTrafficFromServer = new DataInputStream(observerClientSocket.getInputStream());
			
			controlTrafficToServer = new DataOutputStream(server.getOutputStream());
	...
	public void serverRequestDirectory() {
		new Thread( () -> {
			try {
				controlTrafficToServer.writeInt(1);
				controlTrafficToServer.flush();

				physicalTrafficFromServer = new ObjectInputStream(server.getInputStream());

				serverDirectory = (String[])physicalTrafficFromServer.readObject();
	...
	public void serverRequestDownloadFile() {
		new Thread( () -> {
			try {
				controlTrafficToServer.writeInt(2);
				controlTrafficToServer.writeUTF(selectedRemoteFile);

				physicalTrafficFromServer = new ObjectInputStream(server.getInputStream());

				byte[] receivedData = (byte[])physicalTrafficFromServer.readObject();
	...
	public void serverRequestUploadFile() {
		new Thread( () -> {
			try {
				controlTrafficToServer.writeInt(3);
				controlTrafficToServer.writeUTF(selectedLocalFile);
				...
				physicalTrafficToServer = new ObjectOutputStream(server.getOutputStream());
				physicalTrafficToServer.writeObject(fileContent);
				physicalTrafficToServer.flush();
	...
	public void serverRequestDisconnect() {
		try {
            controlTrafficToServer.writeInt(4);
            server.close();
            Platform.exit();
        }
